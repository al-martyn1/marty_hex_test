...
1) В директиве ORG надо сделать чтение сегментного адреса, если задаётся абсолютное значение
2) В директиве ORG при задании относительного адреса через '+' надо использовать виртуал итератор от Memory, 
   он умеет кидать исключение при переполнении
3) При инкрементах смещений элементов также надо использовать кидающие итераторы.
